...
/**
 * FINAL DOPA TRIAL Q5〜Q8（2条件処理プール）：「奇数だけ小さい順に全て押せ！」。
 * FinalAscendingEvensQuestionの対になる問題（奇数版）。
 */
function generate() {
  const odds = pickDistinctOdd(3)
  const evens = pickDistinctEven(randInt(2, 3))
  const numbers = shuffle([...odds, ...evens])
  const order = [...odds].sort((a, b) => a - b)
...
function Component({ spec, onResult }: FinalQuestionComponentProps) {
  ...
  return (
    <QuestionShell instruction={'奇数だけ\n小さい順に全て押せ！'}>
...
export const FinalAscendingOddsModule: FinalQuestionModule = {
  id: 'finalAscendingOdds',
