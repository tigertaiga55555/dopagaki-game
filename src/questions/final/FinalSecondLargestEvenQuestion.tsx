...
/**
 * FINAL DOPA TRIAL Q5〜Q8（2条件処理プール）：「2番目に大きい偶数を押せ！」。
 * 「偶数である」＋「その中で2番目」の複合判断。偶数を4個入れて2番目を一意に決められるようにする。
 */
function generate() {
  const evens = pickDistinct(EVEN_DIGITS, 4)
  const odds = pickDistinct(ODD_DIGITS, randInt(1, 2))
  const numbers = shuffle([...evens, ...odds])
  const sorted = [...evens].sort((a, b) => b - a)
  return { numbers, target: sorted[1] }
}
...
function Component({ spec, onResult }: FinalQuestionComponentProps) {
  ...
  return (
    <QuestionShell instruction="2番目に大きい偶数を押せ！">
...
export const FinalSecondLargestEvenModule: FinalQuestionModule = {
  id: 'finalSecondLargestEven',
