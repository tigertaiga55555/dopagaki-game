...
import { NEW_LOW_MESSAGE } from '../config/messagesV2'
...
export function ResultScreen({ result, onRetry }: Props) {
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center gap-5 px-6 py-8">
      {result.isNewLow && !result.isFirstPlay && (
      ...
      {!result.isFirstPlay && (
        <div className="w-full max-w-xs space-y-1 rounded-2xl bg-white/5 px-4 py-3 text-sm">
          ...
          <div className="flex justify-between text-white/70">
            <span>自己最低</span>
            <span className="font-bold tabular-nums text-white">{result.bestLowPercent}％</span>
          </div>
        </div>
      )}
      ...
      >
        次は{Math.max(0, result.bestLowPercent - 10)}％切れる？
      </button>
    </div>
  )
}
