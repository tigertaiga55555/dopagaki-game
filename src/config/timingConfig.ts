...
export const TIMING_SAFETY = {
  ...
  notifRush: {
    ...
    perRedMs: 520,
    /**
     * Ver.5.0追加修正（重大バグ修正）: 最後の必須赤が出現してから、人間が反応してタップし切る
     * までの最低猶予。実ブラウザPlaywrightで300〜700msの人間ディレイテストを行ったところ、
     * 旧値500msでは反応が700ms付近の遅めのケースでtimeoutに間に合わずMISSになることが
     * 実際に再現された（例: 5個目のクリア直前に3ms差でtimeout）。300〜700msの反応を
     * 確実に許容できるよう900msへ引き上げた。
     */
    reactionBufferMs: 900,
  },
